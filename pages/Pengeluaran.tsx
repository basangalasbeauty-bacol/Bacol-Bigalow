...
import { Plus, ShoppingCart, CheckCircle, Clock, Hash, Download, Upload } from 'lucide-react';
import api from '../services/googleApiService';
import { Pengeluaran as PengeluaranType } from '../types';
...
const formFields = [
    ...
    { name: 'namaBarang', label: 'Nama Barang/Jasa', type: 'text' as const, required: true, className: 'sm:col-span-2 lg:col-span-2' },
    { name: 'jumlahPengeluaran', label: 'Jumlah Pengeluaran', type: 'number' as const, required: true, placeholder: '50000' },
    { name: 'status', label: 'Lunas', type: 'checkbox' as const },
    { name: 'rekanan', label: 'Rekanan', type: 'select' as const, allowAdd: true, className: 'lg:col-span-2' },
...
const Pengeluaran: React.FC = () => {
    const { user } = useAuth();
    const [data, setData] = useState<PengeluaranType[]>([]);
    const [loading, setLoading] = useState(true);
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [editingData, setEditingData] = useState<PengeluaranType | null>(null);
    ...
    const fetchData = useCallback(async () => {
        setLoading(true);
        try {
            const result = await api.getPengeluaran();
    ...
    const filteredData = useMemo(() => {
        return data.filter(item => {
            ...
            return (
                (filters.rekanan ? item.rekanan === filters.rekanan : true) &&
    ...
    const stats = useMemo(() => {
        return {
            total: filteredData.reduce((sum, item) => sum + item.jumlahPengeluaran, 0),
            lunas: filteredData.filter(item => item.status === 'Lunas').reduce((sum, item) => sum + item.jumlahPengeluaran, 0),
            pending: filteredData.filter(item => item.status === 'Pending').reduce((sum, item) => sum + item.jumlahPengeluaran, 0),
    ...
    const handleEdit = (rowData: PengeluaranType) => {
    ...
    const handleSave = async (formData: any) => {
        ...
        try {
            if (editingData) {
                await api.updatePengeluaran(formData);
                setNotification({ message: 'Data berhasil diperbarui!', type: 'success' });
            } else {
                await api.addPengeluaran(formData);
        ...
        } catch (error)
 {
    ...
    const handleExport = () => {
        ...
        exportToExcel(dataToExport, 'Laporan_Pengeluaran');
    };
    ...
    const handleImport = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const transformedData = rawData.map((row: any, index: number): PengeluaranType => {
                ...
                return {
                    ...
                    idTransaksi: row['ID Transaksi']?.toString() || `IMP-K-${Date.now()}-${index}`,
                    namaBarang: row['Nama Barang/Jasa']?.toString() || 'Tidak Diketahui',
                    jumlahPengeluaran: jumlah,
                    rekanan: row['Rekanan']?.toString() || '',
            ...
            await api.overwritePengeluaran(transformedData);
            setNotification({ message: `Berhasil mengimpor ${transformedData.length} data pengeluaran.`, type: 'success' });
            fetchData();
    ...
    const generateId = (date: string) => {
        ...
        return `DK${year}${month}${day}-${String(newCount).padStart(3, '0')}`;
    };

    const columns = [
        { key: 'tanggal', header: 'Tanggal', render: (row: PengeluaranType) => new Date(row.tanggal).toLocaleDateString('id-ID') },
        { key: 'idTransaksi', header: 'ID Transaksi' },
        { key: 'namaBarang', header: 'Nama Barang' },
        { key: 'jumlahPengeluaran', header: 'Jumlah', render: (row: PengeluaranType) => formatCurrency(row.jumlahPengeluaran) },
        { key: 'status', header: 'Status', render: (row: PengeluaranType) => (
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex justify-between items-center">
                <h2 className="text-2xl font-bold text-slate-800">Data Pengeluaran Harian</h2>
                <div className="flex items-center space-x-2">
                    <label htmlFor="import-pengeluaran" className="cursor-pointer flex items-center space-x-2 bg-white text-slate-700 border border-slate-300 px-4 py-2 rounded-lg font-semibold hover:bg-slate-50 transition-colors shadow-sm">
                        <Upload size={20} />
                        <span>Impor Excel</span>
                    </label>
                    <input type="file" id="import-pengeluaran" className="hidden" accept=".xlsx, .xls" onChange={handleImport} />
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 items-center">
                ...
                <select name="rekanan" value={filters.rekanan} onChange={handleFilterChange} className="bg-slate-50 border border-slate-300 text-slate-900 text-sm rounded-lg focus:ring-sky-500 focus:border-sky-500 block w-full p-2.5">
                    <option value="">Semua Rekanan</option>
                    {filterOptions.rekanan.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                </select>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <StatBox title="Jumlah Pengeluaran" value={formatCurrency(stats.total)} icon={<ShoppingCart className="text-red-500"/>}/>
                <StatBox title="Jumlah Lunas" value={formatCurrency(stats.lunas)} icon={<CheckCircle className="text-emerald-500"/>}/>
            ...
            {isModalOpen && (
                <TransactionFormModal
                    ...
                    title={editingData ? 'Edit Data Pengeluaran' : 'Form Input Data Pengeluaran Baru'}
